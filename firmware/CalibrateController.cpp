...
static bool extractLong(JsonVariantConst var, long &out) {
  if (var.is<int>()) {
    out = var.as<int>();
    return true;
  }
...
void CalibrateController::run(const String &message) {
  ...
  if (strcmp(typeCStr, "base_rotation_profile") == 0) {
    ...
    bool ok = ActionBaseRotate::calibrateProfile(jsonOut, neutralServoAngle);
    BuddyMQTT::sendCompletedDetails(actionId, "base_rotation", jsonOut, typeCStr, ok ? "completed" : "failed");
    return;
  }
  ...
  switch (calibType) {
    ...
    default: key = "hover_over_min"; break;
  }
