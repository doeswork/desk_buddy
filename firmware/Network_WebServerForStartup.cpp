...
namespace {
  ...
  void handleSave() {
    ...
    // Save MQTT settings if provided. Blank fields are ignored so changing
    // WiFi does not wipe a working MQTT identity.
    ...
    int mqtt_port_value = mqtt_port.toInt();

    if (mqtt_server.length() > 0 || mqtt_port_value > 0 || mqtt_user.length() > 0 ||
        mqtt_pass.length() > 0 || mqtt_client_id.length() > 0) {
      prefs.begin("mqtt", false);
      if (mqtt_server.length() > 0) prefs.putString("server", mqtt_server);
      if (mqtt_port_value > 0) prefs.putInt("port", mqtt_port_value);
      if (mqtt_user.length() > 0) prefs.putString("user", mqtt_user);
      if (mqtt_pass.length() > 0) prefs.putString("password", mqtt_pass);
      if (mqtt_client_id.length() > 0) prefs.putString("client_id", mqtt_client_id);
      prefs.end();
