...
from .adapter import HuggingFaceZeroShotHandler


def main() -> None:
    parser = argparse.ArgumentParser(description="Desk Buddy Hugging Face detector MQTT worker")
    ...
    handlers = {
        model_id: HuggingFaceZeroShotHandler(
            model_id=model_id,
            source=str(values.get("source", model_id)),
            model_version=str(values.get("version", "unknown")),
